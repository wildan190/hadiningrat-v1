...
@section('rank_math')
    @if(isset($rankMathHead))
        {!! $rankMathHead !!}
    @elseif(isset($product['yoast_head']))
        {!! $product['yoast_head'] !!}
    @elseif(isset($product['head']))
        {!! $product['head'] !!}
    @endif
@endsection
        ...
        <section class="bg-white overflow-hidden">
            <div class="max-w-7xl mx-auto py-8 md:py-16 px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 mb-8 lg:mb-12">
                    ...
                    <div class="space-y-4">
                        @if(!empty($product['images']))
                            <div class="relative rounded-2xl overflow-hidden shadow-xl bg-white">
                                <img src="{{ $product['images'][0]['src'] }}" alt="{{ $product['name'] ?? 'Produk' }}"
                            ...
                            @if(count($product['images']) > 1)
                                <div class="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-4">
                                    @foreach($product['images'] as $image)
                                        <div
                                            class="aspect-square rounded-lg overflow-hidden shadow-md transition-transform hover:scale-105">
                                            <img src="{{ $image['src'] }}" alt="{{ $product['name'] ?? 'Produk' }}"
                        ...
                        @else
                            <div
                                class="w-full h-[300px] sm:h-[400px] lg:h-[500px] bg-gray-100 rounded-2xl flex items-center justify-center">
                    ...
                    <div class="lg:sticky lg:top-32">
                        <div class="bg-white p-4 sm:p-6 lg:p-8 rounded-xl sm:rounded-2xl shadow-lg">
                            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-brand-blue mb-4 break-words">
                                {{ $product['name'] ?? '-' }}</h1>
                            ...
                            @if(!empty($product['categories']))
                                <div class="flex flex-wrap gap-2 mb-6">
                                    ...
                                    @foreach($product['categories'] as $category)
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full bg-gray-100 text-sm font-medium text-gray-800">
                                            {{ $category['name'] }}
                                        </span>
                                    @endforeach
                                </div>
                            ...
                            <div class="space-y-6">
                                <button @click="bookingModal = true" type="button"
                                ...
                                <a href="https://wa.me/6281325794818" target="_blank"
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8 lg:p-10">
                    <h2 class="text-2xl sm:text-3xl font-bold font-title text-brand-blue mb-6 pb-4 border-b">Deskripsi
                        Produk</h2>
                    <div class="prose prose-lg max-w-none">
                        <div
                            class="prose-headings:font-bold prose-headings:text-brand-blue prose-h1:text-3xl prose-h2:text-2xl prose-h3:text-xl prose-h4:text-lg prose-p:text-gray-600 prose-strong:text-brand-blue prose-a:text-brand-orange prose-a:no-underline hover:prose-a:underline prose-ul:list-disc prose-ol:list-decimal prose-li:text-gray-600 prose-img:rounded-xl prose-img:shadow-lg">
                            @php
                    ...
                    <form id="bookingForm" class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="name"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-orange focus:border-transparent text-base"
                                required>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <textarea id="message" rows="3"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-orange focus:border-transparent text-base"></textarea>
                        </div>
                    </form>
                </div>

                <div class="bg-gray-50 px-6 py-4 sm:px-8 rounded-b-lg flex flex-col sm:flex-row gap-3 sm:justify-end">
                    <button @click="bookingModal = false" type="button"
                        class="w-full sm:w-auto px-6 py-2.5 text-gray-700 font-medium border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-200">
                        Tutup
                    </button>
                    <button type="button" id="submitBooking"
                        class="w-full sm:w-auto px-6 py-2.5 bg-brand-orange text-white font-medium rounded-lg hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-brand-orange focus:ring-offset-2 transition-colors">
                        Kirim via WhatsApp
                    </button>
                </div>
            </div>
...
@push('styles')
    <style>
        ...
        /* Menyesuaikan style H2 di dalam deskripsi agar terlihat seperti H1 di editor */
        .prose h2 {
            font-size: 1.875rem;
            /* Ukuran font untuk text-3xl */
            line-height: 2.25rem;
        }
        ...
        /* Fix for offside images in prose */
        .prose img,
        .prose figure,
        .prose .wp-caption {
